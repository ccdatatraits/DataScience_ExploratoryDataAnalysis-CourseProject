## ==ANSWER: Dropping steadily from 1999-2002
##           Staying almost the same from 2002-2005
...
selected_NEI <- select(NEI, SCC, Emissions, year)
selected_SCC <- select(SCC, SCC, Short.Name)
# Merge data by SCC
merged_data <- merge(selected_NEI, selected_SCC, by = "SCC")
# Filter only those cases where Short.Name matches "Coal" (ignore case)
...
# Save to plot4.png using ggsave from ggplot2 package
